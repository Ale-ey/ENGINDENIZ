...
import { getTermsAndConditionsPage } from "@/lib/graphql";
...
export async function generateMetadata(): Promise<Metadata> {
  const pageData = await getTermsAndConditionsPage();
  ...
  if (!pageData) {
    return (
      <main className="min-h-screen bg-white">
        <Header />
        <div className="pt-32 pb-20 px-6 text-center text-red-500">Error: Page not found in WordPress. Please ensure a page named "Terms & Conditions" exists.</div>
      </main>
    );
  }
