...
import { getImprintPage } from "@/lib/graphql";
...
export async function generateMetadata(): Promise<Metadata> {
  const pageData = await getImprintPage();
  ...
  if (!pageData) {
    return (
      <main className="min-h-screen bg-white">
        <Header />
        <div className="pt-32 pb-20 px-6 text-center text-red-500">Error: Page not found in WordPress. Please ensure a page named "Imprint" exists.</div>
      </main>
    );
  }
